...
.collab-a-row1{position:relative;z-index:5;margin-left:28px;margin-top:0;display:flex;align-items:flex-start;gap:130px;align-self:center;color:#fff;font-weight:300;flex-grow:1;overflow:visible}
@media screen and (max-width:1199px){
.collab-a-row1{gap:80px}
}
@media screen and (max-width:1023px){
.collab-a-row1{gap:40px}
}
.collab-a-col-left{position:relative;width:640px;max-width:100%;display:flex;flex-direction:column;gap:0;flex-shrink:0;padding-top:44.8px;padding-bottom:93.1px;align-items:center;align-self:center;overflow:visible}
...
.hero-laptop{--laptop-png-left:-60px;--laptop-png-top:34px;--laptop-png-w:calc(100% + 120px);--laptop-png-h:calc(100% - 65px);position:relative;z-index:2;width:600px;max-width:628px;margin-left:-5%;box-sizing:border-box;overflow:visible;filter:drop-shadow(0 8px 32px rgba(0,0,0,0.25))}
.hero-laptop::before{content:'';display:block;width:100%;padding-bottom:calc(100% * 1031 / 1854 + 55px)}
.hero-laptop-mp4-wrap{position:absolute;left:30px;top:-7px;width:calc(100% - 60px);height:100%;z-index:2;box-sizing:border-box;overflow:hidden;cursor:pointer;display:flex;align-items:center;justify-content:center}
.hero-laptop-mp4-wrap:focus{outline:none}
.hero-laptop-mp4-wrap:focus-visible{outline:2px solid #f15d2f;outline-offset:3px}
.hero-laptop-mp4-wrap .hero-laptop-video{max-width:90%;max-height:90%;width:auto;height:auto;margin:0;padding:0;border:0;background:transparent;object-fit:contain;display:block;vertical-align:top}
.hero-laptop-png-wrap{position:absolute;left:var(--laptop-png-left);top:var(--laptop-png-top);width:var(--laptop-png-w);height:var(--laptop-png-h);z-index:1;box-sizing:border-box;pointer-events:none;line-height:0}
.hero-laptop-png-wrap .hero-laptop-png{width:100%;height:100%;object-fit:fill;object-position:center;display:block;vertical-align:top}
.lp-hero-video-modal{position:fixed;inset:0;z-index:10000;display:none;align-items:center;justify-content:center;padding:max(16px,3vw);box-sizing:border-box;background:rgba(0,0,0,0.42);-webkit-backdrop-filter:blur(3px);backdrop-filter:blur(3px)}
.lp-hero-video-modal.is-open{display:flex}
.lp-hero-video-modal__panel{position:relative;width:75%;max-width:1400px;background:transparent;box-shadow:none}
.lp-hero-video-modal__close{position:absolute;top:12px;right:12px;z-index:2;width:42px;height:42px;display:flex;align-items:center;justify-content:center;padding:0;border:0;border-radius:50%;background:#fff;color:#111;cursor:pointer;box-shadow:0 2px 14px rgba(0,0,0,0.14);transition:transform .18s ease,box-shadow .18s ease,background .18s ease}
.lp-hero-video-modal__close:hover{background:#f8f8f8;transform:scale(1.04);box-shadow:0 4px 20px rgba(0,0,0,0.18)}
.lp-hero-video-modal__close:focus{outline:none}
.lp-hero-video-modal__close:focus-visible{outline:2px solid #f15d2f;outline-offset:3px;background:#f8f8f8;transform:scale(1.04);box-shadow:0 4px 20px rgba(0,0,0,0.18)}
.lp-hero-video-modal__close svg{display:block}
.lp-hero-video-modal__panel video{width:100%;height:auto;display:block;vertical-align:top}
@media screen and (max-width:860px){
.lp-hero-video-modal__panel{width:100%;max-width:none}
}
...
.hero-laptop{width:100%;max-width:480px;margin-left:0}
...
.collab-a-col-left{width:100%;max-width:640px;align-items:center;align-self:center}
...
<div class="collab-a-row1">
  <div class="collab-a-col-left">
    <div class="hero-laptop">
      <div class="hero-laptop-mp4-wrap" id="hero-vibe-video-open" role="button" tabindex="0" aria-label="Open video in larger player" aria-haspopup="dialog">
        <video class="hero-laptop-video" src="{{ '/assets/video/diode-vibe-deploy-example-lp.mp4' | relative_url }}" autoplay muted loop playsinline aria-label="Diode Vibe deploy configuration screen recording"></video>
      </div>
      <div class="hero-laptop-png-wrap" aria-hidden="true">
        <img class="hero-laptop-png" src="{{ '/assets/img/lps/laptop-outline.png' | relative_url }}" alt="" width="1854" height="1031" decoding="async" />
      </div>
    </div>
  </div>
...
<div class="lp-hero-video-modal" id="hero-vibe-video-modal" role="dialog" aria-modal="true" aria-label="Diode Vibe demo video" hidden>
  <div class="lp-hero-video-modal__panel">
    <button type="button" class="lp-hero-video-modal__close" aria-label="Close video"><svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"><path d="M3.5 3.5l7 7M10.5 3.5l-7 7" stroke="currentColor" stroke-width="1.35" stroke-linecap="round"/></svg></button>
    <video src="{{ '/assets/video/diode-vibe-deploy-example-lp.mp4' | relative_url }}" controls playsinline></video>
  </div>
</div>
...
<script>
(function(){
  var openEl=document.getElementById('hero-vibe-video-open');
  var modal=document.getElementById('hero-vibe-video-modal');
  if(!openEl||!modal)return;
  var closeBtn=modal.querySelector('.lp-hero-video-modal__close');
  var heroVideo=openEl.querySelector('video');
  ...
  function openModal(){
    ...
    if(heroVideo&&modalVideo){
      try{modalVideo.currentTime=heroVideo.currentTime;}catch(e){}
      heroVideo.pause();
    }
  ...
  function closeModal(){
    ...
    openEl.blur();
    if(lastFocus&&typeof lastFocus.focus==='function'&&lastFocus!==openEl&&!openEl.contains(lastFocus)){
      try{lastFocus.focus({focusVisible:false});}catch(_){lastFocus.focus();}
    }
    lastFocus=null;
  }
  openEl.addEventListener('click',function(e){e.preventDefault();openModal();});
