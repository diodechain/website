...
This is what we do at Diode for IoT, so when a device connects mqtts://mycloud.diode domain and key information are fetched from the blockchain, because why would you do anything else? The same decentralized PKI foundation also powers the Diode Network and [Diode Collab](https://collab.diode.io/), enabling secure team messaging, decentralized hosting, and a Slack alternative that works as a chatting app without phone number requirements.
