        ...
        <div class="col-md-9 col-sm-9" v-if="accounts.length !== 0">
          <table class="data" id="blockchain-accounts">
            ...
            <tr v-for="acc in accounts">
              <td><% acc.name %></td>
              <td><a :href="'https://moonscan.io/address/' + acc.address"><% acc.address %></a></td>
              <td><% acc.nonce %></td>
              <td v-if="(acc.balance + '').length == 21"><% acc.pbalance %> GLMR</td>
              <td v-else style="color: red"><% acc.balance %></td>
            </tr>
          </table>
        </div>
